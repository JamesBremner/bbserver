...
namespace wex
{
    ...
    */
    ...
    public:
        ...
        void onTcpServerAccept( int client )
        {
            myTcpServerAcceptFunction( client );
        }
        void onTcpServerReadComplete(int port)
        {
            myTcpServerReadCompleteFunction(port);
        }
        bool onQuitApp()
        {
    ...
    */
        ...
        void tcpServerAccept(std::function<void(int)> f)
        {
            myTcpServerAcceptFunction = f;
        }
        /// register function to call when tcp read accurs
        void tcpRead(std::function<void(int port )> f)
        {
    ...
    */
    ...
    private:
        ...
        std::function<void(int)> myTcpServerAcceptFunction;
        std::function<void(int port)> myTcpServerReadCompleteFunction;
        ...
        virtual LRESULT WindowMessageHandler(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
        {
            ...
            if (hwnd == myHandle)
            {
                ...
                case eventMsgID::tcpServerAccept:
                    events().onTcpServerAccept((int)lParam);
                    return true;

                case eventMsgID::tcpServerReadComplete:
                    events().onTcpServerReadComplete((int)lParam);
                    return true;
                }
            }
            else
